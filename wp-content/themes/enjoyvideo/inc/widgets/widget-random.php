 ...
 * @since      1.0.0
 */
class enjoyvideo_Random_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoyvideo-random widget-posts-thumbnail',
			'description' => __( 'Display random posts with thumbnails.', 'enjoyvideo' )
		);

		// Create the widget.
		parent::__construct(
			'enjoyvideo-random',                                   // $this->id_base
			__( '&raquo; Random Posts', 'enjoyvideo' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $random = get_transient( 'enjoyvideo_random_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoyvideo_random_widget_' . $this->id, $random );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoyvideo_random_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Random Posts', 'enjoyvideo' ),
			'limit'     => 5,
			'show_date' => true
		);
