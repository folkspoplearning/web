 ...
 * @since      1.0.0
 */
class EnjoyVideo_Home_Content_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoyvideo-home-block-one',
			'description' => __( 'Display post content blocks. Only use for the "Home Content" widget area.', 'enjoyvideo' )
		);

		// Create the widget.
		parent::__construct(
			'enjoyvideo-home-block-one',          // $this->id_base
			__( '&raquo; Home Content', 'enjoyvideo' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		echo wp_kses_post( $before_widget );
			...
			$query = apply_filters( 'enjoyvideo_home_one_column_args', $args );
				...
				<div class="content-block content-block-1 clear">
					...
					<?php
						if ( ( ! empty( $instance['title'] ) ) && ( $cat_id != 0 ) ) {
							...
							echo '<span class="section-more-link"><a href="' . esc_url( $cat_link ) . '">'. esc_html( 'More', 'enjoyvideo' ) . '</a></span>';

						} elseif ( $cat_id == 0 ) {

							echo '<h3 class="section-title"><span>' . wp_kses_post( apply_filters( 'widget_title',  esc_html( 'Latest Posts', 'enjoyvideo' ), $instance, $this->id_base ) ) . '</span></h3>';
					...
					<div <?php post_class( $grid_col ); ?>>
						...
						<?php if ( has_post_thumbnail() ) { ?>
							<a class="thumbnail-link" href="<?php the_permalink(); ?>">
								...
								<?php if( (enjoyvideo_has_embed_code() || enjoyvideo_has_embed()) ) { ?>
						...
						<div class="entry-header">
							<?php if ( $cat_id == 0 ) { ?>
								<div class="entry-category"><?php enjoyvideo_first_category(); ?></div>
							<?php } ?>
							<h2 class="entry-title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h2>					
							<div class="entry-meta">
								<span class="entry-date"><?php echo esc_html( human_time_diff(get_the_time('U'), current_time('timestamp')) ) . ' '.  esc_html( 'ago', 'enjoyvideo' ); ?></span>
							</div><!-- .entry-meta -->		
						</div><!-- .entry-header -->
	...
	?>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'title' ) ); ?>">
				<?php esc_html_e( 'Title:', 'enjoyvideo' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'cat' ) ); ?>"><?php esc_html_e( 'Choose category', 'enjoyvideo' ); ?></label>
			<select class="widefat" id="<?php echo esc_html( $this->get_field_id( 'cat' ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'cat' ) ); ?>" style="width:100%;">
				...
				<option value="0"><?php esc_html_e( 'All categories &hellip;', 'enjoyvideo' ); ?></option>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id('col') ); ?>">
				...
				<?php esc_html_e( '3 Columns Layout', 'enjoyvideo' ); ?>		    	
			</label>		    
		<br/>
			<label for="<?php echo esc_html( $this->get_field_id('col') ); ?>">
		    	...
		    	<?php esc_html_e( '4 Columns Layout', 'enjoyvideo' ); ?>
			</label>		    
	    </p>

		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'limit' ) ); ?>">
				<?php esc_html_e( 'Number of posts to show', 'enjoyvideo' ); ?>
			</label>
