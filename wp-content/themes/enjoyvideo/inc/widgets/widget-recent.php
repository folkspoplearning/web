 ...
 * @since      1.0.0
 */
class enjoyvideo_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoyvideo-recent widget-posts-thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'enjoyvideo' )
		);

		// Create the widget.
		parent::__construct(
			'enjoyvideo-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'enjoyvideo' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'enjoyvideo_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoyvideo_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoyvideo_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'enjoyvideo' ),
			'limit'     => 5,
			'show_date' => true
		);
